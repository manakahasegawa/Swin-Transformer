...
class ELSA(nn.Module):
    ...
    def __init__(self, n_head=8, kernel_size=3, channel_size=3):
        super().__init__()
        self.hdamard_a = HadamardAttention()
        self.ghost_head = GhostHead()
...
class HadamardAttention(nn.Module):
    ...
    def cal_hp_rk(self, hp):
        return torch.einsum('bchw,cgk->bgkhw', hp, self.rk)

    def cal_rq_hp(self, hp):
        w, h = hp.shape(-1), hp.shape(-2)
        hp = self.unfold_op(hp).view(-1, self.channel_size, self.kernel_size*self.kernel_size, w, h)
        return torch.einsum('cgk,bgkhw', self.rq, hp)
...
class GhostHead(nn.Module):
    ...
    def forward(self, h_attn):
        h_attn = h_attn.unsqueeze(1)

        mul_matrix = self.mul_matrix.view(1, self.channel_size//self.n_head, self.n_head, 1, self.kernel_size*self.kernel_size)
        add_matrix = self.add_matrix.view(1, self.channel_size//self.n_head, self.n_head, 1, self.kernel_size*self.kernel_size)

        h_attn = (mul_matrix ** self.lambda_) * h_attn + self.gamma * add_matrix
        return h_attn.view(-1, self.channel_size, self.n_head, self.kernel_size*self.kernel_size)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = ELSA(self.num_heads, window_size, self.dim)

        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.layer_norm = nn.nn.LayerNorm(dim)

    def forward(self, x):
        inputs = x
        x = self.layer_norm(x)
        out1 = self.attn(x, x, x) + inputs
        x = self.layer_norm(out1)
        out = self.mlp(x) + out1
        return out
